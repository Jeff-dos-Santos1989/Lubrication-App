/* form.js — QA/QC Checklist page logic
   ...
   (() => {
    const __FORM_JS_VERSION = '2025-11-05c'; // cache-bust
  ...
  `.trim().split('\n');
    ...
    // --- Resilient CSV loader (tries appCore first, then fetch locally)
    ...
    // Tiny CSV parser (no quoted commas)
    ...
    async function init() {
      ...
      // Try to wire custom autocomplete, but never let failures block the page
      try {
        ...
        console.warn('Autocomplete wiring failed (continuing):', e);
      }
  
      // Always wire plain input/change so gate works even without autocomplete
      ...
      // Evaluate initial value (e.g., if prefilled)
